...
def oneway_serialize_query(query):
    """Gets a string representation of a query that's guaranteed to be
    identical for identical queries and unique in general"""
    parts = []
    index_list = query.index_list()
    for ix in index_list:
        ix_dict = {
            'kind': ix.kind(),
            'ancestor': ix.has_ancestor(),
            'properties': []
        }
        ...
        for name, direction in ix.properties():
            ...
            if direction == db.Index.DESCENDING:
                prop_dict['direction'] = 'DESCENDING'
            else:
                prop_dict['direction'] = 'ASCENDING'
            ix_dict['properties'].append(prop_dict)

        parts.append(ix_dict)
